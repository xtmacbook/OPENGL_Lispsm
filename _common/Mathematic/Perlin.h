...
public:
	...
	template<class T>
	static void createFieldWithOctave123(T& data, const unsigned X, const unsigned Y, const unsigned baseFreq) {
		typedef T::ElementType Tuppel;
		typedef Tuppel::ElementType D;
		const unsigned CNT = 4;
		data.resize(X,Y);
		unsigned freq = baseFreq;
		const float baseFact = 1.0f/sqrt((float)X*Y);
